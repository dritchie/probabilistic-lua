...
PlotAcfExperiment <- function(filename, my_title) {
	...
	plot(1:1000, means, type="l", ylim=c(min(means)-.1,max(means)+.1), ylab="Autocorrelation", xlab="Lag")
	title(my_title)
...
PlotAcaExperiment <- function(filename, my_title) {
	...
	plot(1:9*5, means, type="l", ylim=c(0,6000),
		ylab="Autocorrelation area", xlab="Number of sites")
	title(my_title)
	lines(1:9*5, means+err, col=2, lty=3)
	lines(1:9*5, means-err, col=2, lty=3)
	dev.off()
}

PlotAcfExperiment("acf_normal", "ACF Normal, 6x6 Ising, 1000 samples, 20 runs")
PlotAcfExperiment("acf_global", "ACF Global tempering, 6x6 Ising, 1000 samples, 20 runs")
PlotAcfExperiment("acf_local", "ACF Local tempering, 6x6 Ising, 1000 samples, 20 runs")
...
plot(1:1000, acf_normal_means, type="l", ylim=c(-1,40), ylab="Autocorrelation", xlab="Lag", col=2)
title("ACF, 6x6 Ising, 1000 samples, 20 runs")
lines(acf_global_means, col=3)
lines(acf_local_means, col=4)
legend(x=700, y=36, legend=c("Normal", "Global", "Local"), col=c(2, 3, 4), lty=c(1, 1, 1))
dev.off()
...
plot(1:9*5, aca_normal_means, type="l", ylim=c(0,6000), ylab="Autocorrelation area", xlab="Number of sites", col=2)
title("ACA, 10-100 sites, 1000 samples, 10 runs")
lines(1:9*5, aca_global_means, col=3)
lines(1:9*5, aca_local_means, col=4)
